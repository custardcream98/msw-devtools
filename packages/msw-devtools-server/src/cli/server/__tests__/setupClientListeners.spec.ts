...
describe("setupClientListeners", () => {
  ...
  it("should timeout if no ack received before CONNECTION_TIMEOUT", () => {
    vi.useFakeTimers({
  ...
  it("should update mock list file if no file on server, and client has sent initial mock list", () => {
    const ws = {
      on: vi.fn(),
      off: vi.fn(),
      send: vi.fn()
    ...
    vi.mocked(eventGuard).mockImplementation((_, __, callback) => {
      callback(MOCKED_MOCK_LIST_FROM_CLIENT as any)
    })
  ...
  it("should not update mock list file if there's file on server, and client has sent initial mock list", () => {
    const ws = {
      on: vi.fn(),
      off: vi.fn(),
      send: vi.fn()
