...
- **요청 로그로 실시간 모니터링.** 인터셉트된 모든 요청을 실시간으로 확인하고, 매치되지 않은 요청에서 원클릭으로 Mock을 생성할 수 있습니다. ([🔗](#요청-로그로-실시간-모니터링))
- 한국어를 지원합니다.
...
Log 탭에서 인터셉트된 모든 요청을 실시간으로 확인할 수 있습니다.

<p align="center">
  <img width="500" src="./image5.png">
</p>

- **실시간 요청 로깅** — 타임스탬프, HTTP 메서드, 상태 코드, 응답 시간 표시
- **매치/미매치 요청** 시각적 구분
- **상세 뷰** — 요청/응답 body 확인
- **원클릭 Mock 생성** — 미매치 요청에서 "이 URL로 Mock 추가" 버튼으로 바로 Add Mock 탭으로 이동
- **녹화 토글** — 캡처 일시정지/재개
- **필터** — 전체, 매치됨, 미매치 필터링
